...
const ITEMS = [
  { key: 'changePin', Icon: PinIcon, gradient: 'from-blue-500 to-indigo-500' },
  { key: 'changePassword', Icon: LockIcon, gradient: 'from-indigo-500 to-violet-500' },
  { key: 'biometric', Icon: FingerprintIcon, gradient: 'from-fuchsia-500 to-pink-500' },
  { key: 'devices', Icon: DeviceIcon, gradient: 'from-cyan-500 to-blue-500' },
  { key: 'freezeCard', Icon: SnowflakeIcon, gradient: 'from-sky-400 to-cyan-500' },
  { key: 'twoFactor', Icon: KeyIcon, gradient: 'from-emerald-500 to-teal-500' },
]
  ...
  return (
    <div className="pb-4">
      ...
      <div className="flex flex-col gap-3">
        {ITEMS.map(({ key, Icon, gradient }, i) => {
          const labelKey = `security.${key}`
          return (
            <button
              key={key}
              onClick={() => onNavigate('placeholder', { labelKey, Icon, gradient })}
